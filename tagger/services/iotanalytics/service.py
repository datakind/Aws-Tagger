...
class iotanalyticsTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.iotanalytics = _client('iotanalytics', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        resource_arn = f'dataset/{resource_arn}'
        ...
        if not self.dryrun:
            try:
                self._iotanalytics_create_tags(resourceArn=file_system_id, tags=aws_tags)
    ...
    def _iotanalytics_create_tags(self, **kwargs):
        return self.iotanalytics.tag_resource(**kwargs)