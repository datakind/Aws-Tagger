...
class ioteventsiotanalyticsTagger(object):
    def __init__(self, dryrun, verbose, servicetype, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.iotevents = _client('iotevents', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags,role=None, region=None):
        ...
        service = "iotevents"
        if self.servicetype == 'IoTEventsDetectorModel':
            resource_arn = "detectorModel/"+resource_arn

        if self.servicetype == 'IoTEventsInput':
        ...
        if not self.dryrun:
            try:
                self._iotevents_create_tags(resourceArn=file_system_id, tags=aws_tags)
    ...
    def _iotevents_create_tags(self, **kwargs):
        return self.iotevents.tag_resource(**kwargs)