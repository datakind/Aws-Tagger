...
class emrcontainersTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.emrcontainers = _client('emr-containers', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        resource_arn = f'virtualclusters/{resource_arn}'
        ...
        if not self.dryrun:
            try:
                self._emrcontainers_create_tags(resourceArn=file_system_id, tags=tags)
    ...
    def _emrcontainers_create_tags(self, **kwargs):
        return self.emrcontainers.tag_resource(**kwargs)