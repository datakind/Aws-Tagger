...
class sqsTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.sqs = _client('sqs', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        if not self.dryrun:
            try:
                self._sqs_create_tags(QueueUrl=resource_arn, Tags=tags)
    ...
    def _sqs_create_tags(self, **kwargs):
