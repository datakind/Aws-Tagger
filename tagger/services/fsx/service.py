...
class fsxTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.fsx = _client('fsx', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        service = 'fsx'
        resource_arn = f'file-system/{resource_arn}'
        ...
        if not self.dryrun:
            try:
                self._fsx_create_tags(ResourceArn=file_system_id, Tags=aws_tags)
    ...
    def _fsx_create_tags(self, **kwargs):
        return self.fsx.tag_resource(**kwargs)