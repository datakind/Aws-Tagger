...
class cognitoTagger(object):
    def __init__(self, dryrun, verbose, servicetype, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.congnito = _client('cognito-identity', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags,role=None, region=None):
        ...
        if self.servicetype == 'CognitoIdentityPool':
            service = "cognito-identity"
            resource_arn = "identitypool/"+resource_arn

        if self.servicetype == 'CognitoUserPool':
            service = "cognito-idp"
            resource_arn = "userpool/"+resource_arn
        ...
        if not self.dryrun:
            try:
                self._cognito_create_tags(Resources=file_system_id, Tags=tags)
    ...
    def _cognito_create_tags(self, **kwargs):
