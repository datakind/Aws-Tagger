...
class sesTagger(object):
    def __init__(self, dryrun, verbose, servicetype, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.sesv2 = _client('sesv2', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags,role=None, region=None):
        ...
        service = "ses"
        if self.servicetype == 'SESConfigurationSet':
            resource_arn = "configuration-set/"+resource_arn

        if self.servicetype == 'SESContactList':
            resource_arn = "contact-list/"+resource_arn

        if self.servicetype == 'SESDedicatedIpPool':
            resource_arn = "dedicated-ip-pool/"+resource_arn

        if self.servicetype == 'SESIdentity':
        ...
        if not self.dryrun:
            try:
                self._sesv2_create_tags(ResourceArn=file_system_id, Tags=aws_tags)
    ...
    def _sesv2_create_tags(self, **kwargs):
        return self.sesv2.tag_resource(**kwargs)