...
class dataexchangeTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.dataexchange = _client('dataexchange', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        resource_arn = f'data-sets/{resource_arn}'
        ...
        if not self.dryrun:
            try:
                self._dataexchange_create_tags(ResourceArn=file_system_id, Tags=tags)
    ...
    def _dataexchange_create_tags(self, **kwargs):
        return self.dataexchange.tag_resource(**kwargs)