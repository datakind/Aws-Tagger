...
class elasticinferenceTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.elasticinferencee = _client('elastic-inference', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        resource_arn = f'elastic-inference-accelerator/{resource_arn}'
        ...
        if not self.dryrun:
            try:
                self._elasticinferencee_create_tags(resourceArn=file_system_id, tags=tags)
    ...
    def _elasticinferencee_create_tags(self, **kwargs):
        return self.elasticinferencee.tag_resource(**kwargs)