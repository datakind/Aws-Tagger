...
class comprehendTagger(object):
    def __init__(self, dryrun, verbose, servicetype, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.comprehend = _client('comprehend', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags,role=None, region=None):
        ...
        service = "comprehend"
        if self.servicetype == 'ComprehendDocumentClassifier':
            resource_arn = "document-classifier/"+resource_arn

        if self.servicetype == 'ComprehendEntityRecognizer':
            resource_arn = "entity-recognizer/"+resource_arn
        ...
        if not self.dryrun:
            try:
                self._comprehend_create_tags(Resources=file_system_id, Tags=aws_tags)
    ...
    def _comprehend_create_tags(self, **kwargs):
        return self.comprehend.tag_resource(**kwargs)