...
class eventsTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.events = _client('events', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        service = 'events'
        resource_arn = f'rule/{resource_arn}'
        ...
        if not self.dryrun:
            try:
                self._events_create_tags(ResourceArn=file_system_id, Tags=aws_tags)
    ...
    def _events_create_tags(self, **kwargs):
        return self.events.tag_resource(**kwargs)
