...
class eksTagger(object):
    def __init__(self, dryrun, verbose, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.role = role
        self.region = region
        self.eks = _client('eks', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        if not self.dryrun:
            try:
                self._eks_create_tags(resourceArn=file_system_id, tags=tags)
    ...
    def _eks_create_tags(self, **kwargs):
        return self.eks.tag_resource(**kwargs)