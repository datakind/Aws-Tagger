...
class ramTagger(object):
    def __init__(self, dryrun, verbose, role=None, region=None):
        self.dryrun = dryrun
        self.verbose = verbose
        self.role = role
        self.region = region
        self.ram = _client('ram', role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        service = 'ram'
        resource_arn = f'resource-share/{resource_arn}'
        ...
        if not self.dryrun:
            try:
                self._ram_create_tags(resourceShareArn=file_system_id, tags=tags)
    ...
    def _ram_create_tags(self, **kwargs):
        return self.ram.tag_resource(**kwargs)