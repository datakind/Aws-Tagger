...
class stepfunctionsTagger(object):
    def __init__(self, dryrun, verbose, servicetype, accesskey, secretaccesskey, role=None, region=None):
        ...
        self.stepfunctions = _client('stepfunctions', accesskey=accesskey, secretaccesskey=secretaccesskey, role=role, region=region)
    ...
    def tag(self, resource_arn, tags,role=None, region=None):
        ...
        if self.servicetype == 'StepFunctionsActivity':
            resource_arn = "activity:"+resource_arn

        if self.servicetype == 'StepFunctionsStateMachine':
            resource_arn = "stateMachine"+resource_arn
        ...
        if not self.dryrun:
            try:
                self._stepfunctions_create_tags(resourceArn=file_system_id, tags=aws_tags)
    ...
    def _stepfunctions_create_tags(self, **kwargs):
        return self.stepfunctions.tag_resource(**kwargs)