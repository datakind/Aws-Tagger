...
class codepipelineTagger(object):
    ...
    def tag(self, resource_arn, tags):
        ...
        if self.servicetype == 'CodePipelinePipeline':
            pass

        if self.servicetype == 'CodePipelineWebhook':
            resource_arn = "webhook:"+resource_arn
        ...
        if not self.dryrun:
            try:
                self._codepipeline_create_tags(resourceArn=file_system_id, tags=aws_tags)
    ...
    def _codepipeline_create_tags(self, **kwargs):
